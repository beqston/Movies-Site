...
const Home = () => {
    ...
    const [hoverIndex, setHoverIndex] = useState<number | null>(null);  // Combined state for hover effect

    const { addBookmark } = useContext(BookmarkContext) as BookmarkContextType;

    const handlePlayHover = useCallback((index: number) => {
        setHoverIndex(index);
    }, []);
            ...
            if (item.isTrending) {
                return (
                    <div
                        ...
                        onMouseOver={() => handlePlayHover(index)}
                        className={classname["trending-items"]}
                    >
                        <Link to={`http://localhost:5173/movie/${item.title}`}>
                            <picture>
                                <img
                                    src={`http://localhost:5173/${item.thumbnail.trending?.large}`}
                                    alt="photo"
                                />
                                <source
                                    media="(min-width: 768px) and (max-width: 1024px)"
                                    srcSet={`http://localhost:5173/${item.thumbnail.trending?.large}`}
                                />
                                <source
                                    media="(max-width: 768px)"
                                    srcSet={`http://localhost:5173/${item.thumbnail.trending?.small}`}
                                />
                            </picture>
                        </Link>
                        ...
                        <div className={classname["title"]}>
                            ...
                            <h2 className={classname["bottom"]}>{item.title}</h2>
                        </div>
        ...
        <main className={classname["home"]}>
            ...
            <div className={classname["main-content-cnt"]}>
                ...
                <section className={classname["trending-cnt"]}>
                    <h2>Trending</h2>
                    <article className={classname["trend"]}>{renderTrendingItems()}</article>
                </section>
